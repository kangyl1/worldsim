...
# What an event makes perceivable, and under what conditions. This suite owns
# the event->knowledge DEFINITION; who actually notices belongs to Selective
# Perception and lives in perception_test.gd. The fixtures place their witnesses
# in Aster on purpose, because that is where these events happen.

const WorldSimulationScript = preload("res://scripts/world_sim.gd")

# The template's own clarity, before the perception pathway takes its cut.
const TEMPLATE_CONFIDENCE := 90
const LOCAL_CONFIDENCE := 81
...
func _init() -> void:
	...
	if completed != EXPECTED_TESTS:
		...
		quit(1)
		return
	print("EVENT KNOWLEDGE TEST PASSED: events offer facts, repeats refresh, beliefs reach intents.")
	quit(0)


func _test_drought_generates_fact() -> void:
	var simulation = _witnessed_world()
	...
	assert(opportunities.size() == 2, "each entity gets one chance, judged separately")
	for entity_id in ["aster_king", "mara"]:
		...
		assert(int(record["confidence"]) == LOCAL_CONFIDENCE)
	...
	print("  DROUGHT: both witnesses learned aster_food_shortage at %d." % LOCAL_CONFIDENCE)
	completed += 1
...
func _test_absorbed_event_teaches_nothing() -> void:
	...
	simulation.state.current_event_id = "drought"
	assert(simulation.tick_perception().is_empty())
...
func _test_repeat_refreshes_same_record() -> void:
	...
	var first: Array[Dictionary] = simulation.tick_perception()
	assert(bool(first[0]["perceived"]))
	assert(simulation.state.get_perceptions_for("aster_king").size() == 1)
	...
	assert(int(stale["confidence"]) < LOCAL_CONFIDENCE, "the belief should have decayed first")
	...
	assert(int(refreshed["confidence"]) == LOCAL_CONFIDENCE)
...
func _test_generated_fact_spreads_as_rumor() -> void:
	...
	assert(int(received["confidence"]) < LOCAL_CONFIDENCE, "hearsay must cost confidence")
	...
	print("  RUMOR: generated fact travelled Mara %d -> Merchant %d." % [
		LOCAL_CONFIDENCE, int(received["confidence"])
	])
	completed += 1
...
func _test_surplus_requires_real_surplus() -> void:
	...
	assert(simulation.tick_perception().is_empty(), "a thin harvest is not a surplus")

	simulation.state.food_level = 2
	var learned: Array[Dictionary] = simulation.tick_perception()
	...
	for turn in 16:
		...
		for intent: Dictionary in simulation.state.last_intents:
			...
			if not intent["knowledge_used"].is_empty():
				knowledge_driven += 1
	assert(facts_generated > 0, "the running world must let somebody notice something")
...
# These events all happen in Aster, so a witness has to live there. Mara is a
# Westfield woman by default, and would otherwise learn none of this first-hand.
func _witnessed_world():
	var simulation = _new_simulation()
	assert(simulation.state.set_home_location("mara", "aster"))
	return simulation
